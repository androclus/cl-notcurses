...
5. **Optional: Emacs** is only required if you prefer it and want to run the examples remotely using SLIME » Swank or SLY » Slynk.

6. **Optional: Kitty** or **Wezterm** terminal emulator programs (which seem to be the fastest) but any standard known terminal emulator (or even hardware) with a well-specified terminfo entry should work.
